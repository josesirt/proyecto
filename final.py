...
class DataAnalyzer:
    ...
    def estadisticas_variable(self, nombre_variable):
        if nombre_variable in self.df.columns:
            ...
            # Dibujar un histograma de la distribución de la variable
...
try:
    ...
    for archivo in archivos_a_cargar:
        ...
        try:
            ...
            repo.update_file(filI_path, f"Actualización de {file_path}", content, existing_file.sha, branch="main")
            print(f"{file_path} actualizado en GitHub.")
        except Exception:
            # Si el archivo no existe, crearlo
            repo.create_file(filI_path, f"Creación de {file_path}", content, branch="main")
            print(f"{file_path} creado en GitHub.")
